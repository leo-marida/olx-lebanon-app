...
export const FiltersScreen: React.FC = () => {
  ...
  const [showDynamicModal, setShowDynamicModal] =
    useState<CategoryField | null>(null);
  ...
  useEffect(() => {
    setPriceMinLocal(filters.priceMin ? String(filters.priceMin) : "");
    setPriceMaxLocal(filters.priceMax ? String(filters.priceMax) : "");
  }, []); // only on mount

  // Apply price to store with debounce
  useEffect(() => {
    const timer = setTimeout(() => {
      const min = priceMinLocal !== "" ? Number(priceMinLocal) : undefined;
      const max = priceMaxLocal !== "" ? Number(priceMaxLocal) : undefined;
      console.log("SETTING PRICE RANGE:", min, max);
      setPriceRange(min, max);
    }, 800);
    return () => clearTimeout(timer);
  }, [priceMinLocal, priceMaxLocal]);

  // Clear local state when reset
  useEffect(() => {
